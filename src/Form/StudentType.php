...
class StudentType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        ->addEventListener(FormEvents::PRE_SUBMIT, function (FormEvent $e) {
            $data= $e->getData();
            $form= $e->getForm();
            $cat= isset($data['tipo_identificacion'])?$data['tipo_identificacion']:null;
           
            if ($cat == 'Cedula') {
                $form->add('Cedula',TextType::class,[
                    'label'=>'CÉDULA',
                    'required' => false,
                    'constraints' => [
                        new NotBlank([
                            'message' => 'Rellenar el campo de cedula para completar el registro',
                        ]),
                        new Length([
                            'min' => 10,'max' => 10,
                        ]),
                        new Regex([
                            ...
                            'pattern'=>'/^[0-9,$]*$/',
                            'message' =>"La cedula solo debe tener numeros"
                        
                        ])
                    ],
                ]);
            }elseif($cat =='Pasaporte'){
                $form->add('Cedula',TextType::class,[
                    'label'=>'PASAPORTE',
                    'required' => false,
                    ...
                    'constraints' => [
                        new NotBlank([
                            'message' => 'Rellenar el campo del pasaporte para completar el registro',
                        ]),
                        new Length([
                            'min' => 8,'max' => 9,
                        ]),
                        new Regex([
                            ...
                            'pattern'=>'/^(?=.*[A-Z])(?=.*\d).{6,}$/i',
                            'message' =>"El campo de pasaporte debe tener letras y numeros"
